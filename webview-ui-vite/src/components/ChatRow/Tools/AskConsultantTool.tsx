  ...
  return (
    <>
      <h3 className="text-alt items-center flex gap-1.5">
        {lastMessage.text === message.text ? <Loader2 className="animate-spin size-4" /> : toolIcon("comment-discussion")}
        {message.type === "ask" ? <>Claude wants to ask consultant with the query </> : <>Claude's consultant replied </>}
      </h3>
      <CodeBlock
        code={tool.context}
        language="plaintext"
