...
import type { OpenRouterChatModelId, OpenRouterChatSettings } from "./openrouter-chat-settings"
import { openAIErrorDataSchema, openrouterFailedResponseHandler } from "./openrouter-error"
...
export class OpenRouterChatLanguageModel implements LanguageModelV1 {
	...
	async doStream(
		...
		const { responseHeaders, value: response } = await postJsonToApi({
			...
			failedResponseHandler: openrouterFailedResponseHandler,
			successfulResponseHandler: createEventSourceResponseHandler(openrouterChatChunkSchema),
			abortSignal: options.abortSignal,
			fetch: this.config.fetch,
		})
		...
		return {
			stream: response.pipeThrough(
				new TransformStream<ParseResult<z.infer<typeof openrouterChatChunkSchema>>, LanguageModelV1StreamPart>({
					transform(chunk, controller) {
						...
						const mappedLogprobs = mapOpenRouterChatLogProbsOutput(choice?.logprobs)
