...
import { DiagnosticsHandler } from "../../agent/v1/handlers"
...
export class DiffViewProvider {
	...
	async saveChanges(): Promise<{ newProblemsMessage: string | undefined; userEdits: string | undefined }> {
		...
		Since these are new problems resulting from Kodu's edit, we know they're
		directly related to the work he's doing. This eliminates the risk of Kodu
		going off-task or getting distracted by unrelated issues, which was a problem
		with the previous auto-debug approach. Some users' machines may be slow to
		update diagnostics, so this approach provides a good balance between automation
		and avoiding potential issues where Kodu might get stuck in loops due to
		outdated problem information. If no new problems show up by the time the user
		accepts the changes, they can always debug later using the '@problems' mention.
		This way, Kodu only becomes aware of new problems resulting from his edits
		and can address them accordingly. If problems don't change immediately after
		applying a fix, Kodu won't be notified, which is generally fine since the
		initial fix is usually correct and it may just take time for linters to catch up.
		*/
		const newProblems = DiagnosticsHandler.errorsToString(this.koduDev.diagnosticsHandler.currentErrors, getCwd())
		...
		if (normalizedEditedContent !== normalizedNewContent) {
			...
			const userEdits = createPrettyPatch(this.relPath.toPosix(), normalizedNewContent, normalizedEditedContent)
			return { newProblemsMessage, userEdits }
		} else {
			// no changes to Kodu's edits
	...
	private async openDiffEditor(): Promise<vscode.TextEditor> {
		...
		return new Promise<vscode.TextEditor>((resolve, reject) => {
			...
			vscode.commands.executeCommand(
				...
				uri,
				`${fileName}: ${fileExists ? "Original ↔ Kodu's Changes" : "New File"} (Editable)`
			)
	...
	scrollToFirstDiff() {
		...
		const diffs = diffLines(this.originalContent || "", currentContent)
		let lineCount = 0
...
export const createPrettyPatch = (filename = "file", oldStr?: string, newStr?: string) => {
	...
	const patch = createPatch(filename.toPosix(), oldStr || "", newStr || "")
