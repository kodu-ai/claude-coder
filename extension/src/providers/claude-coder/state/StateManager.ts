...
export class StateManager {
    ...
    constructor(context: ExtensionProvider) {
        ...
        this.errorStateManager = ErrorStateManager.getInstance(context.context)
    }
    ...
    async clearHistory() {
        ...
        this.errorStateManager.resetErrorStates()
    }

    async clearTask() {
        const taskId = this.context.getKoduDev()?.getStateManager()?.state.taskId
        if (taskId) {
            this.errorStateManager.resolveError(taskId)
        }
        await this.provider.getTaskManager().clearTask()
    }

    async validateState() {
        await this.errorStateManager.validateErrorStates()
    }
