...
export async function applyEditBlocksToFile(
  content: string,
  ...
  for (const block of editBlocks) {
    ...
    const result = replaceIgnoringIndentation(
      newContent,
      searchContent,
      replaceContent
    );
    if (result !== null) {
      ...
      if (similarLines) {
        ...
        console.log(
          `Failed to find match for block: ${block.searchContent.slice(
            0,
            100
          )}...`,
          'warn'
...
function findPositionsInContent(
  content: string,
...
export function getEditBlockPositions(
  originalContent: string,
  blocks: EditBlock[]
): EditBlockWithPosition[] {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const positions = findPositionsInContent(originalContent, [
      block.searchContent,
    ]);
...
export function replaceIgnoringIndentation(
  ...
  for (let i = 0; i <= contentLines.length - searchLines.length; i++) {
    ...
    if (strippedContentSlice.join('\n') === strippedSearchLines.join('\n')) {
      ...
      const indentedReplaceLines = adjustIndentationPerLine(
        contentSlice,
        searchLines,
        replaceLines
      );
...
export function adjustIndentationPerLine(
  ...
  for (let idx = 0; idx < replaceLines.length; idx++) {
    ...
    const indentationDifference =
      contentIndentation.length - searchIndentation.length;

    // Adjust replace line indentation
    let newIndentationLength =
      replaceIndentation.length + indentationDifference;
...
export function findBestBlockMatch(
  ...
  if (exactIndex !== -1) {
    const startLine =
      fileContent.substring(0, exactIndex).split('\n').length - 1;
    return {
      start: startLine,
