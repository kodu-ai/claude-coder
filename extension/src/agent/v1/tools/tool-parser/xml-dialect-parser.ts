...
/**
 * Context interface for XML parsing state
 */
...
/**
 * An XML dialect parser that processes XML syntax for tool commands
 */
export class XmlDialectParser extends BaseDialectParser implements ITextDialectParser {
	/**
	 * The type of dialect this parser handles
	 */
	readonly dialectType = "xml" as const
	private currentContext: XmlContext | null = null
	...
	/**
	 * Returns whether the parser is currently inside a tool tag
	 */
	...
	private finalizeTool(context: XmlContext): void {
		this.validateAndFinalizeTool(context.toolName, context.params, context.id, context.ts)
	}
