...
export const startNewTask = async (
	...
	if (parsedConfig.parsed?.KODU_API_KEY) {
		...
		Run individual tests when you can, but if not possible, run the full test suite. Always test your changes before finalizing. After you believe you've fixed the issue, run the test suite again to ensure there are no regressions. Achieving zero regressions in the test suite is mandatory. If any test fails, re-evaluate your solution.
